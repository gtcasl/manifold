...
int main(int argc, char** argv)
{
    ...
    assert(sysBuilder.get_y_dim() > 1);

    unsigned expectedLPs = sysBuilder.get_proc_node_size() + sysBuilder.get_y_dim();

    if(N_LPs != expectedLPs) {
        cerr << "Number of LPs must be " << expectedLPs << "!" << endl;
	exit(1);
    }
...
#define REDIRECT_COUT
    ...
    sysBuilder.build_system(SysBuilder_llp::FT_QSIMCLIENT, N_LPs, args, SysBuilder_llp::PART_Y);
