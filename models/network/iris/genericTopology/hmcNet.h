...
#ifndef  MANIFOLD_IRIS_HMCNET_H
#define  MANIFOLD_IRIS_HMCNET_H

#include "../interfaces/genericIrisInterface.h"
#include "../interfaces/hmcIrisInterface.h"
...
class hmcNet
{
	...
	//! @param \c node_lp   router idx to LP mapping
        hmcNet (manifold::kernel::Clock& clk, torus6p_init_params* params, const Terminal_to_net_mapping* mapping, SimulatedLen<T>*, VnetAssign<T>*, int ni_credit_type, vector<int>* node_lp);
        ~hmcNet ();
...
template <typename T>
hmcNet<T>::hmcNet(manifold::kernel::Clock& clk, torus6p_init_params* params, const Terminal_to_net_mapping* mapping, SimulatedLen<T>* slen, VnetAssign<T>* vn, int ni_credit_type, vector<int>* node_lp) :
    x_dim(params->x_dim),
    y_dim(params->y_dim),
    clk_t(clk),
    node_lp(node_lp)
...
//        for ( uint j=0; j< intf_per_router; j++)
          interface_ids.push_back( manifold::kernel::Component::Create< HmcNetworkInterface<T> >(node_lp->at(i), i * intf_per_router, niInit, &i_p_inf) );
          interface_ids.push_back( manifold::kernel::Component::Create< GenNetworkInterface<T> >(node_lp->at(i), i * intf_per_router + 1, niInit, &i_p_inf) );
#ifdef IRIS_TEST
...
template <typename T>
void
hmcNet<T>::connect_interface_routers()
{
    int LATENCY = 1;
...
template <typename T>
void hmcNet<T> :: print_stats(std::ostream& out)
{
