#ifndef MANIFOLD_IRIS_HMCINTERFACE_H
#define MANIFOLD_IRIS_HMCINTERFACE_H
...
template<typename T>
class HmcNetworkInterface : public GenericNitworkInterface 
{
    public:
        //the enums that specifies the ports
        enum { ROUTER_PORT=0, TERMINAL_PORT};
        
        HmcNetworkInterface (unsigned ifid, const NIInit<T>&, inf_init_params*);
        ~HmcNetworkInterface (); 
	...
	#ifdef FORECAST_NULL
        void do_output_to_ni_prediction();
	#endif
	...
	bool try_send_to_ni();
	void send_credit_to_ni(T*); //used only by tock()
	...
	uint64_t stat_packets_in_from_ni;
	uint64_t stat_packets_out_to_ni;
...
template<typename T>
HmcNetworkInterface<T>::HmcNetworkInterface (unsigned ifid, const NIInit<T>& niInit, inf_init_params* i_p) :
	...
	vnet(niInit.vnet)
{
    ...
    // Init stats
    stat_packets_in_from_ni = 0;
    stat_packets_out_to_ni = 0;
...
//! the deconstructor
template<typename T>
HmcNetworkInterface<T>::~HmcNetworkInterface ()
{
    delete arbiter;
}
...
template<typename T>
void
HmcNetworkInterface<T>::handle_router (int port, LinkData* data )
{   
    switch ( data->type ) {
        case FLIT:
            {
...
template<typename T>
void HmcNetworkInterface<T> :: handle_incoming(int, T* pkt)
{
//    manifold::urach:NetworkPacket pkt = (manifold::urach:NetworkPacket)(data)    
    if (pkt->type == CREDIT_PKT) {
	downstream_credits++;
	delete pkt;
	return;
    }
...
cout << "@ " << m_clk->NowTicks() << " >>> mc " << m_nid << " received ST, src= " << pkt->get_src() << " addr= " <<hex<< req->get_addr() <<dec<<endl;
    ...
    //// TODO Do the encapsulation/decapsulation like an MCP cache //////////
...
//	manifold::kernel::Manifold::Schedule(1,&HmcNetworkInterface::process_incoming_credit, this);
//	#ifdef FORECAST_NULL
...
template<typename T>
T*
HmcNetworkInterface<T>::from_flit_level_packet(FlitLevelPacket* flp)
{
    T* message = new T;
...
template<typename T>
void
HmcNetworkInterface<T>::tick ( void )
{
...
template<typename T>
void
HmcNetworkInterface<T>::tock ( void )
{
    ...
    for ( uint i=0; i < no_vcs; i++ )
    {
	    ...
	    manifold::kernel::Manifold::ScheduleHalf(1, &HmcNetworkInterface::send_credit_to_ni, this, credit);
	    #ifdef FORECAST_NULL
...
template<typename T>
void
HmcNetworkInterface<T> :: send_credit_to_ni(T* credit)
{
    #ifdef FORECAST_NULL
...
template<typename T>
void
HmcNetworkInterface<T>::to_flit_level_packet(FlitLevelPacket* flp, T* pkt, manifold::kernel::Ticks_t enter_time)
{
    assert(flp->size() == 0);
...
template<typename T>
bool HmcNetworkInterface<T> :: try_send_to_ni
{
...
template<typename T>
void HmcNetworkInterface<T> :: print_stats(std::ostream& out)
{
    out << "Interface " << id << ":\n";
    out << "  Packets in from terminal: " << stat_packets_in_from_ni << "\n";
    out << "  Packets out to terminal:  " << stat_packets_out_to_ni << "\n";
    ...
    #ifdef IRIS_STATS_T2T_DELAY
    out << "  Avg terminal-to-terminal delay: " << (double)stat_t2t_delay/stat_packets_out_to_ni << "\n";
    #endif
}


template<typename T>
void HmcNetworkInterface<T> :: dbg_print()
{
...
#ifdef FORECAST_NULL
template<typename T>
void HmcNetworkInterface<T> :: remote_input_notify(manifold::kernel::Ticks_t when, void* data, int port)
{
    T* pkt = (T*)data;
...
template<typename T>
void HmcNetworkInterface<T> :: do_output_to_ni_prediction()
{
//stat_pred_called++;
...
#endif // MANIFOLD_IRIS_HMCINTERFACE_H
