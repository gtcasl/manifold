...
bool core_exec_DPM_t::STQ_deallocate_std(struct uop_t * const uop)
{
  ...
  if(!STQ[STQ_head].first_byte_requested)
  {
    ...
    core->Send<ZestoCacheReq*>(core_t::PORT0, request); // sendtick?
    core->stats_n_mem_ops++;
    //printf("sent from zesto STQ, %#x core%d reqid= %d\n", request->addr, core->id, request->req_id);
    //printf("sent de STQ @ %ld  %#x c%d rid= %d\n", preq.start, request->addr, core->id, request->req_id);
...
{
    for(std::map<int, core_t::Preq>::iterator it = core->my_map.begin(); it != core->my_map.end(); ++it) {
        if(core->sim_cycle - (*it).second.start > 10000) {
	    //printf("@ %ld  reqId %d  start= %ld  response over 10000 id= %d \n", core->sim_cycle, (*it).first, (*it).second.start, core->id);
	    break;
	}
    }
  ...
  while (core->m_input_from_cache.size() != 0)
  {
    ...
    {
      core->stats_n_finished_mem_ops++;
      core->stats_mem_op_latency += (core->sim_cycle - preq.start);

      struct uop_t * uop = LDQ[preq.index].uop;

      //printf("------------phy address is %#x core%d Reqid= %d ------------------from LD op \n", request->addr, core->id, request->req_id);
      if(uop != NULL)
      {
    ...
    if(request->type == ZestoCacheReq::STORE) // handling stores
    {
      core->stats_n_finished_mem_ops++;
      core->stats_mem_op_latency += (core->sim_cycle - preq.start);
      ...
      //printf("------------phy address is %#x core%d Reqid= %d ------------------from St op \n", request->addr, core->id, request->req_id);
