...
void core_fetch_DPM_t::step(void)
{
  ...
  while( (PC & byteQ_linemask) == current_line )
  {
    ...
    {
      if(core->current_thread->active == false) //simulation is to end.
          return;
...
void core_fetch_DPM_t :: dbg_dump_IQ(ostream& out)
{
    out << "IQ:\n";
    out << "  num= " << IQ_num << "\n";
    ...
    for(int i=0; i<IQ_num; i++) {
        out << "  " << idx << ": PC= " << hex<< IQ[idx]->fetch.PC <<dec<< "\n";
	idx = (idx + 1) % core->knobs->fetch.IQ_size;
    }
}

void core_fetch_DPM_t :: dbg_dump_predecode(ostream& out)
{
    out << "Predecode:\n";
    for(int i=0; i<core->knobs->fetch.depth; i++) {
        out << "  stage " << i << ":\n";
	for(int j=0; j<core->knobs->fetch.width; j++) {
	    if(pipe[i][j])
		out << "    " << j << ": " << "PC= " <<hex<< pipe[i][j]->fetch.PC <<dec<< endl;
	    else
		out << "    " << j << ": " << "No MOP\n";
	}
    }
...
void core_fetch_DPM_t :: dbg_dump_byteQ(ostream& out)
{
    out << "ByteQ:\n";
    out << "  num= " << byteQ_num << "\n";

    int idx = byteQ_head;
    for(int i=0; i<byteQ_num; i++) {
        out << "    byteQ idx " << idx << " addr= " <<hex<< byteQ[idx].addr << " paddr= " << byteQ[idx].paddr
	    << " fetch_requested@ " << byteQ[idx].when_fetch_requested << " fetched@ " << byteQ[idx].when_fetched
	    << " trans_requested@ " << byteQ[idx].when_translation_requested << " translated@ " << byteQ[idx].when_translated
	    << " num_Mop= " << byteQ[idx].num_Mop << " MopQ_1st_idx= " << byteQ[idx].MopQ_first_index
	    << endl;
	idx = (idx + 1) % core->knobs->fetch.byteQ_size;
    }
}
