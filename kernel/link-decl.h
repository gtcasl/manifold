...
#ifndef MANIFOLD_KERNEL_LINK_DECL_H
#define MANIFOLD_KERNEL_LINK_DECL_H
...
public:
 ...
 LinkOutputBase(Ticks_t lat, int ii, Clock* c, Time_t delay,
   ...
   : latency(lat), inputIndex(ii), clock(c),
    ...
    void SendTime(const T& t, Time_t delay)
    {
	data = t;
	Time_t default_latency = timeLatency;
	timeLatency+=delay;
	ScheduleRxEvent();
	timeLatency = default_latency;
    }
...
//The following 2 templated functions are created to solve a compiler
...
template <typename T>
T Deserialize_helper(const T&, unsigned char* buf)
{
  return Deserialize<T>(buf, 0); 
}

template <typename T>
T* Deserialize_helper(T*, unsigned char* buf)
{
...
public:
  ...
  void Recv(Ticks_t tick, Time_t time, unsigned char* data, int len)
  {
    ...
    this->data = Deserialize_helper(this->data, data);
    ScheduleRxEvent(tick, time);
  }

  virtual void ScheduleRxEvent(Ticks_t tick, Time_t time) = 0;

  //this implementation is NOT correct: what if T is a pointer type?
...
//! Exception class used when sending a wrong data type over a link.
class BadSendTypeException {
};
...
{
public:
    virtual ~LinkBase() {} //must have at least one virtual function. Otherwise, dynamic_cast doesn't work.

    //! Function used to send data on the link.
    //!  @arg The actual data to be sent.
    template<typename T>
    void Send(T t) throw (BadSendTypeException);
    template<typename T>
    void SendTick(T t, Ticks_t delay) throw (BadSendTypeException);
    template<typename T>
    void SendTime(T t, Time_t delay) throw (BadSendTypeException);
